...
public class Rectangle extends CustomShape implements java.io.Serializable {


	private static final long serialVersionUID = -8186505753401198430L;
	protected int width;
	...
	public boolean isInShape(Point start) {
		if (pointInBetween(cntrPnt.x, refPnt.x, start.x) && pointInBetween(cntrPnt.y, refPnt.y, start.y))
			return true;
		return false;
	}
	...
	public void draw(Graphics2D g) {
		...
		if (colorBundle.getHasFill()) {
			...
			sg.fillRect(Math.min(this.cntrPnt.x, this.refPnt.x), Math.min(this.cntrPnt.y, this.refPnt.y), width,
					height);
		}
		if (!isSelected)
			sg.setStroke(new BasicStroke(colorBundle.getStrokeThickness()));
		sg.setColor(colorBundle.getStrokeCol());
		sg.drawRect(Math.min(this.cntrPnt.x, this.refPnt.x), Math.min(this.cntrPnt.y, this.refPnt.y), width, height);
		sg.dispose();
	}
