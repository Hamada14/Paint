...
import ActionManagement.ShapePackage;
...
public class DrawingArea extends JComponent {
	...
	public final static int LINE = 0, TRIANGLE = 1, RECTANGLE = 2, SQUARE = 3, ELLIPSE = 4, CUSTOM = 5;
	...
	public void saveCurrentState(int stateShape, boolean isVisible) {
		...
		if (geomShapes.get(stateShape) instanceof Triangle)
			triPoint = new Point(((Triangle) geomShapes.get(stateShape)).getThirdPoint());
		else
			triPoint = null;
		actionManager.addToHistory(stateShape, cntrPnt, refPnt, triPoint, colorBundle, isVisible, geomShapes);
	}
	...
	public DrawingArea(ColorBundle colorBundle, Toolbar toolbar) {
		geomShapes = new ArrayList<CustomShape>();
		actionManager = new ActionManager();
	...
	private void errorPop(String st) {
		JOptionPane.showMessageDialog(GUIPanel.drawingPad, st, "Error", JOptionPane.ERROR_MESSAGE);
	}
		...
		addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
				...
				if (curState == ADD) {
					...
					triangleVertex = ((Triangle) geomShapes.get(selectedShape)).getNearestPoint(start);
				}
				if (triangleOn) {
					triangleOn = false;
			...
			public void mouseReleased(MouseEvent e) {
				...
				if (curState == ADD && curShape == TRIANGLE && mouseOn) {
					...
					if (selectedShape == -1) {
						...
						((Triangle) geomShapes.get(selectedShape)).resize(triangleVertex, end);
					} else {
	...
	public void drawStatics() {
		if (toolbar.isConnected() || toolbar.isHosting()) {
			drawOnlineStatics();
		}
		drawOflineStatics();
	}

	public void drawOnlineStatics() {
		ShapePackage cur = null;
		if (toolbar.isHosting())
			cur = toolbar.paintServer.getPackage();
		else if (toolbar.isConnected() && toolbar.client != null)
			cur = toolbar.client.getPackage();
		if(cur == null) return;
		int vSize = cur.getSize();
		for (int i = 0; i < vSize; i++) {
			cur.get(i).draw(graphic);
		}
	}

	private void drawOflineStatics() {
		int vSize = geomShapes.size();
		for (int i = 0; i < vSize; i++) {
			if ((curState == RESIZE && mouseOn && selectedShape == i) || !actionManager.isShapeVisible(i))
				continue;
			...
			if (mouseOn) {
				if (curState == ADD)
					...
					if (geomShapes.get(selectedShape) instanceof Triangle) {
						((Triangle) geomShapes.get(selectedShape)).resize(triangleVertex, mid);
					} else {
	...
	public boolean loadClass(File file) {
		...
		boolean isLoaded = ls.load(str, fileName.substring(0, fileName.length() - 4));
		if (isLoaded)
			return true;
		return false;
	}
	...
	public ShapePackage getShapePackage() {
		ShapePackage sp = new ShapePackage();
		int vSize = geomShapes.size();
		for (int i = 0; i < vSize; i++) {
			if (!actionManager.isShapeVisible(i))
				continue;
			sp.add(geomShapes.get(i));
		}
		return sp;
	}
}
